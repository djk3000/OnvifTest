...
int Yolo_pose::load(const char* modeltype,int _target_size, const float* _mean_vals, const float* _norm_vals, bool use_gpu) {
	...
	char parampath[256];
	char modelpath[256];
	sprintf(parampath, "yolov8n-pose-new-sim-opt.param", modeltype);
	sprintf(modelpath, "yolov8n-pose-new-sim-opt.bin", modeltype);
...
int Yolo_pose::load(AAssetManager* mgr, const char* modeltype, int _target_size, const float* _mean_vals, const float* _norm_vals, bool use_gpu)
{
	...
	char parampath[256];
	char modelpath[256];
	sprintf(parampath, "yolov8s-pose-opt-fp16.param", modeltype);
	sprintf(modelpath, "yolov8s-pose-opt-fp16.bin", modeltype);
...
int** Yolo_pose::getDirectPoints (const std::vector<Object_pose>& objects) {
    const int num_point = 17;
    int** array = new int*[objects.size()];
	__android_log_print(ANDROID_LOG_DEBUG, "ncnn", "getDirectPoints 1111111:%d" ,objects.size());
    for (int s= 0; s<objects.size(); s++) {
        Object_pose obj = objects[s];
        array[s] = new int[55];
        array[s][0] = obj.rect.x;
		array[s][1] = obj.rect.y;
		array[s][2] = obj.rect.width;
		array[s][3] = obj.rect.height;
        ...
        for (int k = 0; k < num_point + 2; k++){
            if (k < num_point){
                ...
                if (kps_s > 0.5f){
                    array[s][k*3+4] = k;
                    array[s][k*3+1+4] = kps_x;
                    array[s][k*3+2+4] = kps_y;
                } else {
                    array[s][k*3+4] = k;
                    array[s][k*3+1+4] = 0;
                    array[s][k*3+2+4] = 0;
		...
		for (int k = 0; k < num_point + 2; k++)
		{
			if (k < num_point)
			{
				...
				if (kps_s > 0.5f)
				{
					...
					array[s][k*3+1] = kps_x;
					array[s][k*3+2] = kps_y;
//					__android_log_print(ANDROID_LOG_INFO,"native-lib","##person kps=%d",k);
//					__android_log_print(ANDROID_LOG_INFO,"native-lib","##person kps_x=%d",kps_x);
//					__android_log_print(ANDROID_LOG_INFO,"native-lib","##person kps_y=%d",kps_y);
				} else {
					array[s][k*3] = k;
					array[s][k*3+1] = 0;
					array[s][k*3+2] = 0;
